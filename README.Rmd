...
def parse_data(data_splitted, i):
  ...
  # Append if there's a different length
...
#to_be_shown = df_to_markdown(df)

to_be_shown = df.to_markdown(index=False)
print(to_be_shown)
...
for entry in json_data:
  ...
  grid_entry = f'<a href="{blog_url}"><img width="100" alt="Image of {name}" src="{photo_url}">&nbsp;</a><span class="caption">{name}</span>|{sep}'
  
  grid_entries += grid_entry
  
to_be_returned = f"{header}<br>{first_row}<br>|{grid_entries}"
